<template>
  <div class="pay">
    ...
    <div class="address">
      ...
      <div class="info" v-if="selAdress?.address_id">
        <div class="info-content">
          <span class="name">{{selAdress.name}}</span>
          <span class="mobile">{{selAdress.phone}}</span>
        </div>
    ...
    <div class="pay-list" v-if="order.goodsList">
      ...
      <div class="pay-detail">
        ...
        <div class="pay-cell">
          <span>配送费用：</span>
          <span v-if="!selAdress">请先选择配送地址</span>
...
import { checkOrder, subOrder } from '@/api/order'
export default {
  name: 'PayIndex',
  data () {
    return {
  ...
  methods: {
    async getAdressList () {
      const { data: { list } } = await getAdress()
  ...
  computed: {
    selAdress () {
      return this.adressList[0]
    },
    longAdress () {
      const region = this.selAdress.region
      return region.province + region.city + region.region + this
        .selAdress.detail
    },
    mode () {

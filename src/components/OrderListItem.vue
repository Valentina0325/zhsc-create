  ...
  <div class="order-list-item" v-if="item.order_id">
    ...
    <div class="actions">
      <div v-if="item.order_status === 10">
        ...
        <span v-else-if="item.delivery_status === 10" @click="cancel(item.order_id)">申请取消</span>
...
<script>
import { cancelList } from '@/api/order'
...
export default {
  ...
  methods: {
    async  cancel (orderId) {
      try {
        await cancelList(orderId)
        this.$emit('cancle-sucess')
        Toast('取消成功')
      } catch (error) {
